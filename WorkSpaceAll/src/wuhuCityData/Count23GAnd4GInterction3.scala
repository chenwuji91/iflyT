...
/**
  ...
  */
object Count23GAnd4GInterction3 {
  def main(args: Array[String]) {
    ...
    println(g4.count())
    val map234g = sc.textFile("hdfs://192.168.86.41:9000/user/wjchen/4GIMSI_23GIMSI_MAP.txt")
      .map(x => x.split("\t")).map(x => (x(0),x(1))).cache()

    val piPeiDao = g23.intersection(map234g.map(x=>x._2)).distinct()
    println(piPeiDao.count())
//    val afterJoin = piPeiDao.map(x=>(x," "))(map234g);
    deleteHDFSDir("hdfs://192.168.86.41:9000/user/wjchen/test41/t.txt")
  //  afterJoin.saveAsTextFile("hdfs://192.168.86.41:9000/user/wjchen/test41/t.txt")
//    println("sss"+ afterJoin.count())
